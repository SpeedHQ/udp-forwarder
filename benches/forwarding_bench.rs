...
const GAMES: &[Game] = &[
    Game { name: "Forza Motorsport", packet_size: 331 },
    Game { name: "ACC",             packet_size: 608 },
...
fn main() {
    ...
    // Header
    print!("{:<12}", "");
    for game in GAMES { print!("{:>16}", game.name); }
    println!();
    print!("{:<12}", "");
    for _ in GAMES { print!("{:>16}", "---"); }
    println!();

    // Packet size row
    print!("{:<12}", "Bytes");
    for game in GAMES { print!("{:>16}", game.packet_size); }
    println!();

    // Delivery
    print!("{:<12}", "Delivery");
    for r in &results { print!("{:>15}%", format!("{:.1}", r.delivery_pct)); }
    println!();
    ...
    for (label, vals) in &rows {
        print!("{:<12}", label);
        for v in vals { print!("{:>14}", format!("{:.1}µs", v)); }
        println!();
    }
}
...
fn run_test(
    ...
    let safe_name: String = game.name.chars().map(|c| if c.is_alphanumeric() { c } else { '-' }).collect();
    ...
    println!("  Throughput: {}/{} ({:.1}%) in {:.2}s", total_received, expected_total, delivery_pct, elapsed.as_secs_f64());
    ...
    if latencies.is_empty() {
        ...
        return TestResult { sent: total_packets, delivery_pct, avg_us: 0.0, p50_us: 0.0, p95_us: 0.0, p99_us: 0.0, max_us: 0.0 };
    }
    ...
    println!("  Latency: Avg {:.1}µs  P50 {:.1}µs  P95 {:.1}µs  P99 {:.1}µs  Max {:.1}µs",
        avg as f64 / 1000.0, p50 as f64 / 1000.0, p95 as f64 / 1000.0,
        p99 as f64 / 1000.0, max as f64 / 1000.0);

    TestResult {
        sent: total_packets, delivery_pct,
        avg_us: avg as f64 / 1000.0, p50_us: p50 as f64 / 1000.0,
        p95_us: p95 as f64 / 1000.0, p99_us: p99 as f64 / 1000.0,
